...
class CombineCamerasNode(Node):
    def __init__(self):
        ...
        self.declare_parameter("marker_size", 0.05)             # meters (5 cm measured per marker)
        ...
        # table_z: height of the table surface in the robot world frame.
        # With constrain_to_table=True, z is forced to (table_z + marker_height_above_table).
        # After z_sign=-1.0 the published z becomes -(table_z + marker_height_above_table).
        # Set so that -(table_z + 0.005) equals the actual marker plane z in robot world frame.
        # Example: markers at z=+0.05 world → table_z = -0.055  (then -(−0.055+0.005) = +0.050).
        self.declare_parameter("table_z", -0.055)               # meters — see above
        self.declare_parameter("constrain_to_table", True)      # project coordinates to flat table plane
        self.declare_parameter("marker_height_above_table", 0.005) # meters (board/marker thickness height, e.g. 5mm)

        # Axis sign corrections — set to -1.0 to flip an axis if the output
        # coordinate has the wrong sign in the robot world frame after the
        # camera→world TF transform.
        #
        # Current camera (global RealSense D435i, eye-to-base calibrated TF):
        #   Raw X is already positive when the container is in front of the robot
        #   → x_sign = +1.0  (no flip needed)
        #   Raw Y sign matches world Y convention after TF
        #   → y_sign = +1.0  (no flip needed; adjust if container drifts left/right)
        #   Raw Z is negative (camera frame) → world Z positive (above table)
        #   → z_sign = -1.0  (flip needed)
        #
        # If you replace the camera or redo the hand-eye calibration, re-check
        # the "raw:" value in the "Center pipeline" log.  If an axis has the
        # wrong sign in "final:", flip its sign parameter here.
        self.declare_parameter("x_sign",  1.0)   # +1.0 or -1.0
        self.declare_parameter("y_sign",  1.0)
        self.declare_parameter("z_sign", -1.0)

        # Per-axis bias offsets (metres, in robot world frame, applied AFTER sign correction).
        # Use these to correct systematic errors from hand-eye calibration imprecision.
        # Measure the residual error between the fused output and ground-truth position,
        # then set the offset to the negative of that error:
        #   Example: camera reads x=+0.24 but true x=+0.30  →  x_offset = +0.06
        #   Example: camera reads y=+0.08 but true y=0.00   →  y_offset = -0.08
        #   Example: camera reads z=+0.05 but true z=+0.05  →  z_offset =  0.00
        # Note: z error is usually better fixed via the table_z parameter above.
        self.declare_parameter("x_offset", 0.0)   # metres
        ...
        self.constrain_to_table = self.get_parameter("constrain_to_table").value
        self.marker_height_above_table = self.get_parameter("marker_height_above_table").value

        # Axis sign corrections (±1.0 each)
        ...
        if set([abs(self.x_sign), abs(self.y_sign), abs(self.z_sign)]) - {1.0}:
            self.get_logger().warn(
                "x_sign / y_sign / z_sign should be +1.0 or -1.0. "
                f"Got: x={self.x_sign}, y={self.y_sign}, z={self.z_sign}.")

        # Per-axis bias offsets (metres, robot world frame)
    ...
    def _detect_aruco_markers(self, gray_img: np.ndarray):
        ...
        Performance note: the detector and dictionary objects are created once
        at startup and cached in self._aruco_detector / self._aruco_legacy_*
        so we avoid re-allocating them on every frame (was a ~2 ms overhead
        at 10 Hz × 3 cameras).
        """
        if hasattr(self, "_aruco_detector"):
            # Modern path — cached ArucoDetector
            return self._aruco_detector.detectMarkers(gray_img)
        if hasattr(cv2, "aruco") and hasattr(cv2.aruco, "ArucoDetector"):
            # Build and cache on first call
            dictionary = cv2.aruco.getPredefinedDictionary(self.aruco_dict_id)
            params = cv2.aruco.DetectorParameters()
            # ── Accuracy tuning ──────────────────────────────────────────────
            # Subpixel corner refinement: improves PnP accuracy by ~0.3 px.
            # CORNER_REFINE_SUBPIX is the best option for planar markers.
            params.cornerRefinementMethod = cv2.aruco.CORNER_REFINE_SUBPIX
            params.cornerRefinementWinSize   = 5
            params.cornerRefinementMaxIterations = 30
            params.cornerRefinementMinAccuracy  = 0.1
            # Adaptive threshold tuning — improves detection under mixed lighting.
            params.adaptiveThreshConstant    = 7
            params.adaptiveThreshWinSizeMin  = 3
            params.adaptiveThreshWinSizeMax  = 23
            params.adaptiveThreshWinSizeStep = 10
            # Min perimeter avoids false positives from noise.
            params.minMarkerPerimeterRate = 0.03
            self._aruco_detector = cv2.aruco.ArucoDetector(dictionary, params)
            return self._aruco_detector.detectMarkers(gray_img)
        else:
            # Legacy OpenCV — build cache on first call
            if not hasattr(self, "_aruco_legacy_dict"):
                self._aruco_legacy_dict   = cv2.aruco.Dictionary_get(self.aruco_dict_id)
                self._aruco_legacy_params = cv2.aruco.DetectorParameters_create()
                self._aruco_legacy_params.cornerRefinementMethod = (
                    cv2.aruco.CORNER_REFINE_SUBPIX
                    if hasattr(cv2.aruco, "CORNER_REFINE_SUBPIX") else 0)
            return cv2.aruco.detectMarkers(
                gray_img, self._aruco_legacy_dict,
                parameters=self._aruco_legacy_params)

    def _estimate_single_marker_pose(self, corners: np.ndarray, marker_size: float,
                                     K: np.ndarray, D: np.ndarray):
        """
        Estimate 3D position [x,y,z] of a single marker using PnP.

        Uses SOLVEPNP_IPPE_SQUARE when available (OpenCV ≥ 4.0).  This
        method is specifically designed for square planar targets and is
        more accurate than the generic iterative solver (SOLVEPNP_ITERATIVE)
        at distances > ~30 cm.  It also returns two candidate solutions;
        we select the one with the smaller reprojection error.
        """
        obj_points = np.array([
        ...
        pts_2d = corners.astype(np.float32)

        # IPPE_SQUARE returns two hypotheses; pick by reprojection error
        if hasattr(cv2, "SOLVEPNP_IPPE_SQUARE"):
            try:
                n_sol, rvecs, tvecs, reproj_errs = cv2.solvePnPGeneric(
                    obj_points, pts_2d, K, D,
                    flags=cv2.SOLVEPNP_IPPE_SQUARE)
                if n_sol >= 1:
                    best = int(np.argmin([e[0] for e in reproj_errs]))
                    return True, rvecs[best], tvecs[best]
            except Exception:
                pass  # fall through to standard solver

        # Fallback: standard iterative solver (all OpenCV versions)
    ...
    def _fuse_and_publish_center(self):
        ...
        # 1. Filter out stale measurements and fuse valid ones per corner.
        #
        # Fusion strategy:
        #   • 1 camera  : use directly (no outlier rejection needed)
        #   • 2 cameras : use distance-weighted average (closer camera = more pixels
        #                 = lower PnP noise)
        ...
        for m_id in self.corner_ids:
            ...
            if n == 1:
                ...
                d0 = float(np.linalg.norm(valid_positions[0]))
                d1 = float(np.linalg.norm(valid_positions[1]))
                # Closer = higher weight (inverse distance)
                w0 = 1.0 / max(d0, 0.001)
                w1 = 1.0 / max(d1, 0.001)
                fused_pos = (w0 * valid_positions[0] + w1 * valid_positions[1]) / (w0 + w1)
            else:
                # 3+ cameras: element-wise median (robust to single-camera outliers)
                fused_pos = np.median(np.array(valid_positions), axis=0)

            if self.constrain_to_table:
                fused_pos[2] = self.table_z + self.marker_height_above_table
            fused_corners[m_id] = fused_pos
        ...
        # Apply axis sign corrections.  These flip individual world-frame
        # coordinates that the camera optical frame convention inverts.
        # Example: a forward-facing camera whose +Z maps to world -X needs
        # x_sign = -1.0 so the container appears at a positive X.
        cx = self.x_sign * float(center[0])
        cy = self.y_sign * float(center[1])
        cz = self.z_sign * float(center[2])

        # Apply per-axis bias correction (hand-eye calibration residual compensation).
        # These offsets are in robot world frame and are added after the sign flip.
        cx += self.x_offset
        cy += self.y_offset
        cz += self.z_offset
        ...
        self.get_logger().info(
            f"Center pipeline — "
            f"raw: [{center[0]:.4f}, {center[1]:.4f}, {center[2]:.4f}]  "
            f"sign: [{self.x_sign * float(center[0]):.4f}, "
            f"{self.y_sign * float(center[1]):.4f}, "
            f"{self.z_sign * float(center[2]):.4f}]  "
            f"final: [{cx:.4f}, {cy:.4f}, {cz:.4f}]",
    ...
    if node.enable_visualization:
        ...
        try:
            ...
            try:
                ...
                pass  # rclpy's own signal handler may have already called shutdown
